...
def ensure_custom_provider(sections: list[tuple[str, list[str]]], base_url: str, api_key: str, models: list[str], requires_auth: bool = False) -> list[tuple[str, list[str]]]:
    updated: list[tuple[str, list[str]]] = []
    found = False
    for section, lines in sections:
        if section == f"[model_providers.{CUSTOM_PROVIDER_ID}]":
            found = True
            # v0.5.1 authenticates the desktop provider with a provider-level
            # bearer token. Leaving an older env_key beside it can make Codex
            # resolve the wrong credential source.
            lines = remove_key(lines, "env_key")
            lines = set_key(lines, "base_url", base_url)
            lines = set_key(lines, "name", CUSTOM_PROVIDER_ID)
            lines = set_bool_key(lines, "requires_openai_auth", requires_auth)
...
def rewrite_config_for_local(content: str, base_url: str, model: str, api_key: str, catalog_path: Path, models: list[str] | None = None, skip_login: bool = False) -> str:
    all_models = models if models else [model]
    ...
    rewritten = ensure_custom_provider(rewritten, base_url, api_key, all_models, requires_auth=not skip_login)
...
def rewrite_config_for_parallel(
    ...
    all_custom_models = custom_models if custom_models else [custom_model]
    sections = split_toml_sections(content)
    rewritten: list[tuple[str, list[str]]] = []
    ...
    for section, lines in sections:
        if section == "":
            ...
            if catalog_path is not None:
                ...
                lines = remove_key(lines, "model_catalog_json")
        rewritten.append((section, lines))
    # When default is custom, skip login; when official, require login
    skip_login = (default_provider == CUSTOM_PROVIDER_ID)
    rewritten = ensure_custom_provider(rewritten, base_url, api_key, all_custom_models, requires_auth=not skip_login)
...
def custom_model_entry(
    ...
    if template is not None:
        ...
        entry["description"] = "Custom model registered by Codex+国产模型免登"
    ...
    return {
        ...
        "description": "Custom model registered by Codex+国产模型免登",
...
class AdapterHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if self.path in {"/models", "/v1/models"}:
            ...
            primary = effective_setting(state, "local_model", DEFAULT_MODEL)
            display = effective_setting(state, "local_model_display_name", primary)
            extra_models = state.get("local_models") or [primary]
            all_slugs: list[str] = []
            seen: set[str] = set()
            for slug in [primary] + list(extra_models):
                if slug and slug not in seen:
                    all_slugs.append(slug)
                    seen.add(slug)
            model_list = []
            for slug in all_slugs:
                name = display if slug == primary else slug
                model_list.append({
                    "id": slug,
                    "object": "model",
                    "created": 1700000000,
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            stream_started = bool(request.get("stream"))
            self.proxy_response(request)
        except Exception as exc:
            if stream_started:
                # SSE 头已发出，用 SSE error event 收尾，不能再发 JSON
                try:
...
def run_adapter(args: argparse.Namespace) -> int:
    ...
    print(f"Codex+国产模型免登 adapter listening on http://{args.host}:{args.port}/v1")
    server.serve_forever()
    return 0


def adapter_launch_agent_path() -> Path:
    return Path.home() / "Library" / "LaunchAgents" / "com.huangama.codex-skip-login.adapter.plist"
...
def write_adapter_launch_agent(home: Path, host: str = DEFAULT_ADAPTER_HOST, port: int = DEFAULT_ADAPTER_PORT) -> Path:
    ...
    log_path = str(home / "codex-skip-login-adapter.log")
    content = f'''<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<dict>
  <key>Label</key>
  <string>com.huangama.codex-skip-login.adapter</string>
...
def start_adapter_launch_agent(home: Path) -> Path:
    ...
    subprocess.run(["launchctl", "kickstart", "-k", f"{launchctl_gui_target()}/com.huangama.codex-skip-login.adapter"], check=False)
    return path
...
def switch_local(args: argparse.Namespace) -> int:
    ...
    # Multi-model support: --models "model1,model2,model3" or from state
    extra_models_str = getattr(args, "models", None) or ""
    if extra_models_str:
        all_models = [m.strip() for m in extra_models_str.split(",") if m.strip()]
        state["local_models"] = all_models
    else:
        all_models = state.get("local_models") or [model]
    # Ensure primary model is in the list
    ...
    use_chat_adapter = bool(getattr(args, "chat_adapter", False))
    if use_chat_adapter:
        # Codex always speaks Responses. The local service translates that to
        # Chat Completions and is the only URL Codex should see.
        state["chat_adapter"] = "true"
        state["adapter_upstream_base_url"] = base_url
        state["adapter_upstream_model"] = model
    ...
    write_model_catalog(catalog_path, model, model, [(m, m) for m in all_models if m != model])
    adapter_plist = start_adapter_launch_agent(home) if use_chat_adapter else None
    config = rewrite_config_for_local(
        read_config(config_path), provider_base_url, model, api_key,
        catalog_path, models=all_models, skip_login=skip_login,
    )
    atomic_write(config_path, config, 0o600)

    print("Switched Codex to custom model API mode.")
    print(f"codex_home: {home}")
    print(f"base_url: {provider_base_url}")
    print(f"upstream_base_url: {base_url}")
    print(f"chat_adapter: {'true' if use_chat_adapter else 'false'}")
    ...
    print(f"requires_openai_auth: {'false' if skip_login else 'true'}")
...
def configure_codex(args: argparse.Namespace) -> int:
    ...
    extra_models_str = getattr(args, "models", None) or ""
    if extra_models_str:
        all_custom_models = [m.strip() for m in extra_models_str.split(",") if m.strip()]
        state["local_models"] = all_custom_models
    else:
        all_custom_models = state.get("local_models") or [custom_model]
    if custom_model not in all_custom_models:
        all_custom_models.insert(0, custom_model)
    cached_key = state.get("local_api_key")
    stdin_key = ""
    if getattr(args, "api_key_stdin", False):
        stdin_key = sys.stdin.readline().strip()
    ...
    auth["auth_mode"] = "chatgpt"
    write_auth(auth_path, auth)
    # Write catalog with all custom models visible
    additional = [(m, m) for m in all_custom_models if m != custom_model] + [(official_model, official_model)]
    write_model_catalog(catalog_path, custom_model, display_name, additional, visible_provider=default_provider)
    ...
    print(f"custom_models: {', '.join(all_custom_models)}")
    print(f"custom_model_name: {display_name}")
    print(f"requires_openai_auth: {not skip_login}")
...
def status(_: argparse.Namespace) -> int:
    ...
    requires_auth = re.search(
        r"^\[model_providers\.custom\][\s\S]*?^requires_openai_auth\s*=\s*(true|false)",
        config,
        re.MULTILINE,
    )
    skip_login = requires_auth is not None and requires_auth.group(1) == "false"
    print(f"skip_login: {'true' if skip_login else 'false'}")
    print(f"upstream_base_url: {effective_setting(state, 'adapter_upstream_base_url', effective_setting(state, 'local_base_url', '(missing)'))}")
...
def config_set(args: argparse.Namespace) -> int:
    ...
    for key, value in updates.items():
        if value is not None:
            ...
            state[key] = stripped
    save_state(home, state)
    print("Saved Codex+国产模型免登 settings.")
...
def claude_config_set(args: argparse.Namespace) -> int:
    ...
    for key, value in updates.items():
        if value is not None:
            ...
            state[key] = stripped
    save_claude_state(home, state)
    print("Saved Claude Code Codex+国产模型免登 settings.")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    local.add_argument("--models", help="Comma-separated list of models to register (e.g. 'gpt-4o,claude-3,glm-4'). All will be available in Codex model picker.")
    local.add_argument("--chat-adapter", action="store_true", help="Translate Codex Responses requests to upstream Chat Completions.")
    local.add_argument("--skip-login", action="store_true", help="Skip ChatGPT login for the custom provider.")
    ...
    config_parser = subparsers.add_parser("config", help="Show or update Codex+国产模型免登 defaults.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"codex-skip-login: {exc}", file=sys.stderr)
        return 2
