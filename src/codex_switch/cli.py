...
def rewrite_config_for_local(content: str, base_url: str, model: str, api_key: str, catalog_path: Path, models: list[str] | None = None) -> str:
    ...
    rewritten = ensure_custom_provider(rewritten, base_url, api_key, all_models, requires_auth=False)
...
def switch_local(args: argparse.Namespace) -> int:
    ...
    config = rewrite_config_for_local(read_config(config_path), base_url, model, api_key, catalog_path, models=all_models)
    atomic_write(config_path, config, 0o600)

    print("Switched Codex to local relay API mode (login bypassed).")
    ...
    print(f"requires_openai_auth: false")
