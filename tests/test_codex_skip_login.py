...
class CodexSwitchTests(unittest.TestCase):
    def test_local_chat_adapter_routes_codex_to_local_responses_and_removes_env_key(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            home = Path(temp) / ".codex"
            write_sample_config(home)
            # Simulate a config left by the env_key-based implementation.
            ...
            with mock.patch.object(cli_module, "codex_home", return_value=home), \
                 mock.patch.object(cli_module, "start_adapter_launch_agent", return_value=Path("/tmp/adapter.plist")):
                self.assertEqual(cli_module.switch_local(args), 0)

            config = config_path.read_text(encoding="utf-8")
            self.assertIn(f'base_url = "{cli_module.DEFAULT_ADAPTER_BASE_URL}"', config)
            self.assertIn('experimental_bearer_token = "sk-test-secret"', config)
            self.assertIn('models = ["glm-test"]', config)
            self.assertIn('requires_openai_auth = false', config)
            self.assertNotIn("env_key", config)
            state = json.loads((home / "codex-switch-state.json").read_text(encoding="utf-8"))
            self.assertEqual(state["adapter_upstream_base_url"], "https://chat.example.com/v1")
            self.assertEqual(state["adapter_upstream_model"], "glm-test")
            self.assertEqual(state["chat_adapter"], "true")
